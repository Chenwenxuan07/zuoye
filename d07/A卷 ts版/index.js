...
function xr(arr) {
    ...
    let numipt = document.getElementsByTagName('input')[1];
    let ym = Number(Math.ceil(array.length / Number(numipt.value)));
    span1.innerHTML = '';
    for (let i = 0; i < ym; i++) {
...
function anxr(ele) {
    if (ele.value != '') {
        let ym = Number(Math.ceil(array.length / Number(ele.value)));
        span1.innerHTML = '';
        for (let i = 0; i < ym; i++) {
            let span = document.createElement('span');
            span.innerHTML = `
